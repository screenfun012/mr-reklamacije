...
**Testing (03, 10):** TDD expected. **Real Postgres in tests** — never mock DB/Zod/domain services/Hono. Always mock time (`vi.setSystemTime`), randomness, external APIs (OpenAI mocked except `[real-api]`). No `skip`/`only`/commented tests. Coverage (CI-enforced): services 90%, repos 85%, controllers 80%, utils 100%, overall API 85%, excel 95%, auth 95%, web components 70% (warn). E2E single-worker, stable selectors (`getByRole`), never `waitForTimeout`. Every bug fix ships a regression test. Test DB must end in `_test` (`assertIntegrationDatabase` refuses dev/non-`_test`).
...
- **`users.account_status`:** reality is ahead of docs — migration `0016` added `account_status` (`pending`/`approved`/`rejected`) and the approval flow assigns the `operator` role on approval (super-admin excluded). `docs/05`/`docs/02` still describe only `is_active` + `client_registration_requests`; the user-approval path is the newer mechanism.
