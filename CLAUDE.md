...
- **Decouple client view-recording from the detail GET (2026-07-18, commits `d36fdeb`, `7a256ec`, shipped on `refactor/decouple-client-view-recording`):** `GET /api/emotive-claims/:id` is now a pure read — `EmotiveClaimsService.findById` no longer has the Phase-3 side effect of upserting the client's view row. "Seen" is recorded only via the new `EmotiveClaimsService.markClientSeen` / `POST /api/emotive-claims/:id/mark-seen` (204, same `view_own_customer` gate as the GET, Primljeno/non-owned → 404, no permission change, no migration, no audit entry). `apps/portal-web` fires `mark-seen` once on real claim-detail mount (`useEffect`, not the route loader — hovering a list card to prefetch no longer counts as "seen") and on success invalidates the client list + summary queries so the dashboard NEW/UPDATE badge clears without a reload. `clientEmotiveClaimDetailOptions` is now `staleTime: 0` + `refetchOnWindowFocus: false` so re-entering a claim recomputes `sectionFreshness` fresh; the per-visit "Novo" markers stay stable because the GET itself never advances `viewed_at` anymore. This supersedes the `1e249e8` unmount-`removeQueries` patch (removed) — that approach cleared the cache but left the dashboard badge stale until a full reload.
