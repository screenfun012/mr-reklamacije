...
- **Portal claims list is client-side pagination over ≤50 (latent cap).** `apps/portal-web/src/routes/claims.tsx` fetches one page of `clientClaimsListOptions()` (`CLIENT_CLAIMS_FETCH_PAGE_SIZE = 50`, `@mr/shared/queries/claims.ts`), then the list (`features/claims/client-claims-list.tsx`) does client-side search + client-side pagination (9/page, `ClientClaimsPagination`). A client with **>50 claims only ever sees their first 50** (a subtle `portal_claims_capped` caption tells them so — shown when the fetch returns exactly 50). **Fix when a client crosses 50:** switch to server-side pagination (fetch page N via `page`/`pageSize` on `/api/claims`, which already supports it) + server-side `search`; then drop the client-side slice + the capped caption. Deliberately deferred — clients have few claims today.
...
- **Connection pool size:** quoted as 10 (`rules/06`/`docs/11`) and 20 (`rules/10`). Minor; verify against `@mr/db` client if it matters.
