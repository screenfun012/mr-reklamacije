...
/**
 * Design-handoff glyphs (cog/check/x) ship as black-on-transparent PNGs and are
 ...
 */
const ICON_SRC = {
  cog: '/portal/icon-cog.png',
  check: '/portal/icon-check.png',
  x: '/portal/icon-x.png',
} as const
...
export function MaskedIcon({
  ...
  if (spinning) {
    return (
      <span className={cn('mrp-spin-cog grid flex-none place-items-center', className)}>
...
export function PortalLogo({ className }: { className?: string }) {
  const mask = "url('/portal/logo-white.png') left center / contain no-repeat"
  return (
    <span
      role="img"
      aria-label="MR Engines"
      className={cn('block bg-mrp-logoc', className)}
