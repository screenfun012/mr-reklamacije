...
| ⑪  | Na kom je jeziku?                             | **Oba, u istom fajlu** — srpski list pa engleski (Nikola, 14.08.). Nema trenutka u kome bi se biralo: jezik papira se bira u pregledu, koji se otvara POSLE potpisa, a tražiti od radnika još jedan izbor je tačno ono što `docs/25` §3.0 zabranjuje. Košta drugu stranu, ne drugi fajl: strana JE list, pa se dva lista prelamaju tačno na šavu, bez biblioteke za spajanje. Papir iz pregleda ostaje jednostran, na izabranom jeziku. |
