...
interface EmotiveClaimSeedRow {
  ...
  customerName: string
  sourceCode: string
  /** Lookup key for engine_types.code (e.g. BMW N47D20D). */
  engineTypeCode: string
  /** Serial code stored on emotive_claims.engine_code (e.g. WW328394203). */
  motorSerialCode?: string | null
  employeeName: string
  dateOfClaim: string
...
const EMOTIVE_CLAIMS: EmotiveClaimSeedRow[] = [
  ...
  },
  {
    ...
    customerName: 'SELMAN',
    sourceCode: 'SELMAN',
    engineTypeCode: 'Range rover 448DT',
    motorSerialCode: 'WW772104556',
  ...
  },
  {
    ...
    engineTypeCode: 'Mercedes OM651',
    motorSerialCode: 'WW903441218',
...
export async function seedEmotiveClaims(db: NodePgDatabase<typeof schema>): Promise<void> {
  ...
  for (const row of EMOTIVE_CLAIMS) {
    ...
    const engineTypeId = await lookupIdByName(db, 'engineTypes', row.engineTypeCode)
    ...
    await db.insert(schema.emotiveClaims).values({
      ...
      engineCode: row.motorSerialCode ?? null,
      engineTypeId,
      dateOfClaim,
      dateOfFinish,
      employeeId,
